...
def add_hex_to_dic(line, current_file):
    ...
    if comp not in g_the_greate_dic:
        g_the_greate_dic[comp] = Company(comp)

    if current_file not in g_the_greate_dic[comp].files:
        g_the_greate_dic[comp].files.append(current_file)

    g_the_greate_dic[comp].addHexMac(mac)
...
def add_base_to_dic(line):
    ...
    g_the_greate_dic[comp].addBaseMac(mac)
...
def save_parsed_data_to_file():
    logging.info('Number of coanies in dictionary: ' + str(len(g_the_greate_dic)))
    headers = 'Company_Names;Occurrence_Counter;MAC_Ranges'

    with open(g_result_file, 'w', encoding='utf8') as wfile:
        wfile.write(headers + '\n')
        for key, value in g_the_greate_dic.items():
            if value.cnt > 1:
...
def main():
    logging.info('check needed directories exist')
    verify_dir()
...
if __name__ == "__main__":
    config_logging()
    logging.info('run Timors shit parser')
    main()
    logging.info('finished running Timors shit....')
